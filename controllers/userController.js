...
const client = require("twilio")(
  "ACa899afaba0ceea5a1cb6a79b4101190b",
  "7ae4addb51bb57124e90dcd1c2874c5e"
);
...
const registerUser = async function (req, res) {
  try {
    ...
    /* This is checking if the file is valid or not. */
    ...
    /* This is storing the name of the file. */

    var profileImage = files[0].originalname;
    /* This is storing the name of the file. */
    let image = `assets/${profileImage}`;
    /* Storing the file data in a variable. */
...
const verifyOtp = async function (req, res) {
  try {
    ...
    /* This is checking if the otp is right or not. */
...
const updateUser = async function (req, res) {
  try {
    ...
    if (!validator.isValidRequestBody(requestBody)) {
      res.status(200).send({
        status: true,
        message: "No paramateres passed. Task unmodified",
      });
      return;
    }
    ...
    /* This is checking if the user is valid or not. If the user is valid then it will check if the
    user has a name or not. If the user has a name then it will update the name. */
